...
export default function PublicProfilePage() {
  ...
  const multisensorProjects = filtered.filter(
  ...
  const allRegions = useMemo(() => {
    ...
    projects.forEach((p) =>
      p.samples.forEach((sm) => {
        if (sm.region) s.add(sm.region);
      })
    );
    return Array.from(s);
  }, [projects]);
    ...
    <div className="profile-page">
      ...
      <div className="profile-layout">
          ...
          <div className="public-stats-panel">
            ...
            <div className="public-stat-row">
              ...
              <span className="public-stat-val">
                {projects.filter((p) => p.system_type === "multisensor").length}
              </span>
            </div>
            <div className="public-stat-row">
              <span>Dosing</span>
              <span className="public-stat-val">
                {projects.filter((p) => p.system_type === "dosing").length}
              </span>
            </div>
          </div>
        ...
        <main className="profile-main">
          ...
          {loading && (
            <p className="no-data" style={{ marginTop: 24 }}>
              Loading projects…
            </p>
          )}
                ...
                {multisensorProjects.map((p) => (
                  ...
                  >
                    <div className="card-top">
                      <span className="card-type-badge multisensor">
                        MultiSensor
                      </span>
                    ...
                    <div className="card-meta">
                      {p.samples.length > 0
                        ? `${p.samples.length} sample${p.samples.length !== 1 ? "s" : ""}`
                        : "—"}
                    </div>
                  </div>
                ))}
            ...
            <div className="profile-section">
              ...
              <div className="profile-projects-grid">
                {dosingProjects.map((p) => (
                  <div
                    key={p.id}
                    className="profile-project-card dosing-card public-card"
                    onClick={() =>
                      nav(`/view-project/${encodeURIComponent(p.id)}`)
                    }
                    style={{ cursor: "pointer" }}
                  >
                    ...
                    <div className="card-meta">
                      {p.samples.length > 0
                        ? `${p.samples.length} source${p.samples.length !== 1 ? "s" : ""}`
                        : "—"}
                    </div>
                  </div>
                ))}
